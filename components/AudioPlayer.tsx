...
export default function AudioPlayer({
  ...
  return (
    <div className="my-8 bg-gradient-to-r from-[#2C221E] via-[#3D2F2A] to-[#2C221E] text-[#FAF6F0] rounded-2xl p-6 shadow-md border border-[#D99B26]/30 relative overflow-hidden">
      ...
      {audioSrc ? (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-white/10">
          ...
          <button
            ...
            disabled={loading}
            className="px-5 py-2.5 bg-[#D99B26] hover:bg-[#c28a21] text-[#2C221E] font-bold text-xs rounded-full shadow-sm transition-all transform hover:-translate-y-0.5 cursor-pointer whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {loading ? (
              <>
                <svg
                  className="animate-spin h-4 w-4 text-[#2C221E]"
                  fill="none"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
      ...
      {error && (
        <div className="mt-3 p-3 bg-red-50 border border-red-200 text-red-800 text-xs rounded-xl flex items-center justify-between gap-2 animate-fade-in" role="alert">
          <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-red-600 hover:text-red-900 text-xs font-bold cursor-pointer"
          >
            ✕
          </button>
        </div>
